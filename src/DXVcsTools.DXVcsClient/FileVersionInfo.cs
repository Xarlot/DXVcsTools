...
namespace DXVcsTools.DXVcsClient
{
    public class FileVersionInfoBase 
    {
        ...
        public FileVersionInfoBase(FileVersionInfoBase copy) 
            : this(copy.Version, copy.Date, copy.User, copy.Comment)
        {
        }
    }
    public class FileVersionInfo : FileVersionInfoBase
    {
        ...
        public FileVersionInfo(int version, DateTime date, string user, string comment, byte[] data)
            : base(version, date, user, comment)
        {
            if (data == null)

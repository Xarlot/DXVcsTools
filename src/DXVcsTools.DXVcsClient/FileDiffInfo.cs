...
namespace DXVcsTools.DXVcsClient
{
    ...
    {
        string[] baseData;
        DiffStringItem[][] deltas;
        FileVersionInfoBase[] historyInfo;
        ...
        {
            List<IBlameLine> result = new List<IBlameLine>();
        ...
        static void ApplyDelta(List<IBlameLine> annotation, DiffStringItem[] delta, FileVersionInfoBase fileVersionInfo)
        {
            foreach (var item in delta)
            {
                ...
                if (item.InsertedB > 0)
                {
                    annotation.InsertRange(item.StartB, new List<string>(item.Inserted).ConvertAll(
                        x => (IBlameLine)new BlameLine(fileVersionInfo, x)
                    ));
                }
            }
        }
    ...
    {
        FileVersionInfoBase versionInfo;
        string sourceLine;
